...
.. code-block:: console
   ...
   for word in "${data[@]}"; do
       mesg=$(curl -XGET --header 'Content-Type: application/json' http://localhost:9200/rse*/_search -d '{ "size": 10000, "sort": { "R_ISODATE": "desc" }, "query": { "range": { "@timestamp": { "gte": "now-1h/h" } } } }' | jq -r -c '.hits.hits[]._source | "\(.DATE) \(.MESSAGE)"' | grep $word)
       if [ -z "$mesg" ]
       then
           echo "No variable!"
       else
...
.. code-block:: console
   ...
   then
       ...
       echo "" >> /opt/rs.txt
       for word in "${data[@]}"; do
           curl -XGET --header 'Content-Type: application/json' http://localhost:9200/rse*/_search -d '{ "size": 10000, "sort": { "R_ISODATE": "desc" }, "query": { "range": { "@timestamp": { "gte": "now-1h/h" } } } }' | jq -r -c '.hits.hits[]._source | "\(.DATE) \(.MESSAGE)"' | grep $word >> /opt/rs.txt
       done
       echo "" >> /opt/rs.txt
       echo "The messages above are generated by the <hostname>!" >> /opt/rs.txt
       echo "" >> /opt/rs.txt
