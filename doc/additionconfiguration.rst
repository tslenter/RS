...
.. code-block:: console
   ...
   for word in "${data[@]}"; do
       mesg=$(curl -s -XGET --header 'Content-Type: application/json' http://localhost:9200/rse*/_search -d '{ "query" : { "bool" : { "must": [ { "match": { "MESSAGE": "module" } }, { "range": { "@timestamp": { "gte": "now-1d/d" } } } ] } } , "size": 100 }' | /usr/bin/jq | grep $word)
       if [ -z "$mesg" ]
       then
           echo "No variable!"
       else
...
.. code-block:: console
   ...
   then
       ...
       echo "" >> /opt/rs.txt
       for word in "${data[@]}"; do
           curl -s -XGET --header 'Content-Type: application/json' http://localhost:9200/rse*/_search -d '{ "query" : { "bool" : { "must": [ { "match": { "MESSAGE": "module" } }, { "range": { "@timestamp": { "gte": "now-1d/d" } } } ] } } , "size": 100 }' | /usr/bin/jq | grep $word >> /opt/rs.txt
       done
       echo "" >> /opt/rs.txt
       echo "The messages above are generated by the <hostname>!" >> /opt/rs.txt
       echo "" >> /opt/rs.txt
